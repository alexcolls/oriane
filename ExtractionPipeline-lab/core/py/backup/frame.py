...
# ─── Config ──────────────────────────────────────────────────────────────────
INPUT_DIR = "../tmp/cropped"
OUTPUT_DIR = "../output"
MIN_FRAMES = 3  # at least this many frames per video
THRESHOLD = 0.2  # FFmpeg scene threshold
...
def ffmpeg_extract_with_pts(video_path: str, out_dir: str, threshold: float):
    ...
    pattern = os.path.join(out_dir, "%d.png")
    cmd = [
        FFMPEG_PATH,
        "-hide_banner",
        "-loglevel",
    ...
    entries = []
    for p in glob(os.path.join(out_dir, "*.png")):
...
def process_video(path: str):
    ...
    for old_path, frame_no in entries:
        ...
        new_name = f"{idx}_{seconds:.2f}.png"
    ...
    if idx <= MIN_FRAMES:
        ...
        for frame_no in (i * step for i in range(1, MIN_FRAMES + 1)):
            ...
            seconds = frame_no / fps
            out_name = f"{idx}_{seconds:.2f}.png"
