...
# Simple EKS Node Group Fix Script
# This script creates a working node group without launch templates

set -e

echo "=== Simple EKS Node Group Fix ==="
...
NODE_ROLE_NAME="EKS-Simple-NodeGroup-Role-$(date +%s)"
...
echo "=== Step 4: Creating new node group ==="
NEW_NODE_GROUP_NAME="simple-nodes-$(date +%Y%m%d-%H%M%S)"
...
# Create node group using just the first 2 subnets
